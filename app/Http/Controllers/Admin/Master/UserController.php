...
class UserController extends Controller
{
    ...
    {
        try {
            // Cek apakah user memiliki transaksi shuttlecock
            if ($user->shuttleTransactions()->exists()) {
                return response()->json([
                    'success' => false,
                    'message' => 'User tidak dapat dihapus karena masih memiliki transaksi shuttlecock.'
                ]);
            }

            // Cek apakah user memiliki pemesanan lapangan
            if ($user->orders()->exists()) {
                return response()->json([
                    'success' => false,
                    'message' => 'User tidak dapat dihapus karena masih memiliki pemesanan lapangan.'
                ]);
            }

            // Hapus jika tidak ada keterkaitan data
            $user->delete();
            return response()->json([
                'success' => true,
