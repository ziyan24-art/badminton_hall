@extends('layouts.app', [
'namePage' => 'Kelola Merk Bola',
'class' => 'login-page sidebar-mini ',
'activePage' => 'ball',
'backgroundImage' => asset('now') . "/img/bg14.jpg",
'parent' => 'master'
])
@section('title','Merk Bola')
@section('content')
...
<script>
    $(document).ready(function() {
        ...
        $('.btn-delete').click(function() {
            ...
            confirmDelete(`Anda yakin ingin menghapus Jenis Bola?`, function() {
                $.ajax({
                    url: `/admin/master/ball/delete/${id}`,
                    type: 'DELETE',
                    dataType: 'json',
                    data: {
                        _token: token
                    ...
                    success: function(data) {
                        if (data?.success) {
                            toastr.success(data?.message || 'Berhasil menghapus');
                            return setTimeout(() => {
                                window.location.reload();
                            }, 800);
                        }
                        return toastr.error(data?.message || 'Gagal menghapus data');
                    },
                    error: function(xhr, status, err) {
                        return toastr.error(err.toString());
                    }
