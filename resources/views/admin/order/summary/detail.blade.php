@extends('layouts.app', [
'namePage' => 'Rekap Orderan '.$order->bdm_field->name,
'class' => 'login-page sidebar-mini',
'activePage' => 'rekap',
'backgroundImage' => asset('now') . "/img/bg14.jpg",
'parent' => 'order'
])
@section('title','Detail Order '.$order->bdm_field->name)
@section('content')
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <p class="text-muted">{{ $order->bdm_field->name }}</p>
                    </div>
                    <div class="d-flex justify-content-between align-items-center">
                    ...
                    <a href="#" onclick="window.print()" class="d-print-none btn btn-success btn-round w-100 mr-2">
            ...
            <div class="card d-print-block">
                <div class="card-body description-box">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <p class="text-muted">
                            {{ $transaction->payment_type->bank_name ?? 'Cash / Tunai' }}
                        </p>
                    </div>
                    <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <p class="text-muted">
                            <span class="badge badge-{{ $transaction->is_valid ? 'success' : 'danger' }}">
                                {{ $transaction->is_valid ? 'Pembayaran Valid' : $transaction->order->status_transaction->name_admin }}
                            </span>
                        </p>
                    </div>
                </div>
                <div class="card-footer d-flex justify-content-between d-print-none">
                    <button type="button" data-id="{{ $transaction->id }}" class="btn btn-primary btn-round w-100 btn-view mr-2">
...
@push('modal')
<div class="modal fade" id="modalEdit" tabindex="-1" role="dialog" aria-labelledby="modalFormTitle" aria-hidden="true">
            ...
            <div class="modal-body">
                <form action="" method="post" id="editForm">
                    ...
                    <div class="form-group">
                        ...
                        <label>Tagihan</label>
                        <p class="h4 mt-n2 text-info" id="paymentTotal">Rp. 0</p>
                    </div>
                    <div class="form-group">
...
<script>
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data?.success) {
                        ...
                        const total = parseFloat(trx?.code || 0) + parseFloat(trx?.amount || 0);
                        $('#paymentTotal').html(`Rp. ${total.toLocaleString('id-ID')}`);
                        ...
                        toastr.error(data?.message || 'Gagal memuat data');
                    }
                },
                error: function(xhr, status, err) {
                    toastr.error('Terjadi kesalahan: ' + err);
                }
            });
        });
        ...
        $('#editForm').submit(function(e) {
            e.preventDefault();
            return swallConfirm(this, 'Apakah Anda yakin ingin mengubah status transaksi?', 'Ya, Simpan');
        });
    });
</script>
@endpush