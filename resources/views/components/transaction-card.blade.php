<div class="card-transaction md:w-1/2 w-full">
    ...
    <div class="card-header">
        <h2 class="text-xl font-bold text-gray-600">
            {{ optional($order->bdm_field)->name ?? 'Lapangan tidak tersedia' }}
        </h2>
    </div>
    ...
    <div>
        ...
        <div class="flex justify-between items-center">
            ...
            <span>
                <p class="py-1 px-3 text-white text-xs font-medium {{ optional($order->status_transaction)->color ?? 'bg-gray-400' }} rounded-md">
                    {{ optional($order->status_transaction)->name_user ?? 'Status tidak diketahui' }}
                </p>
            </span>
        </div>
        <div class="flex justify-between items-center">
            <p class="text-gray-400 font-medium"> Total </p>
