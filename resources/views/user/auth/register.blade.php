...
<div class="mt-5 h-full align-middle flex justify-center items-center flex-col">
    <div class="bg-white shadow-xl px-4 py-4 rounded-lg w-full md:max-w-sm">
        ...
        <form action="{{ route('register') }}" method="post" id="formRegister" autocomplete="off">
            @csrf
            <div class="mb-3">
                <label>Nama Lengkap</label>
                <input type="text" class="bg-gray-100" name="name" placeholder="Name" autocomplete="new-name">
            </div>
            <div class="mb-3">
                <label>Email</label>
                <input type="email" class="bg-gray-100" name="email" placeholder="Email" autocomplete="new-email">
            </div>
            <div class="mb-3">
                <label class="block">WhatsApp</label>
                <input type="tel" class="bg-gray-100" name="phone" inputmode="tel" placeholder="8xxx-xxx-xxx">
            </div>
            <div class="mb-3">
                <label>Kata Sandi</label>
                <input type="password" class="bg-gray-100" name="password" placeholder="Kata Sandi" autocomplete="new-password">
            </div>
            <div class="mb-3">
        ...
        $('input[name=phone]').keyup(function(e) {
            ...
            if (match.length > 0) {
                match.map((value, index) => {
                    str += (index + 1 >= match.length) ? `${value}` : `${value}-`;
                })
            }
            $(this).val(str);
        });
