...
<div class="flex justify-between {{ $imageExist ? 'space-x-2' : 'flex-col' }} mb-3" id="images">
    ...
    <div class="{{ $imageExist ? 'w-1/3' : 'hidden' }}">
        <div class="flex flex-col space-y-4 h-32 md:h-64 overflow-y-auto overflow-x-hidden">
            ...
            <img src="{{ asset($image->img) }}" class="object-cover rounded w-full img-detail cursor-pointer transition-all duration-300" alt="Foto Lapangan badminton">
            @endforeach
        </div>
...
<div id="description" class="my-3 text-black font-semibold">
    <p style="color: black;">
    ...
    <p class="flex items-center flex-wrap gap-2">
        <img src="{{ asset('icon/badminton.svg') }}" alt="Shuttlecock" class="w-5 h-5">
        <span class="text-sm text-black font-semibold">Merk Bola Tersedia:</span>

        @foreach ($ball_types as $ball)
        <span class="text-xs px-2 py-1 rounded-full text-black bg-green-600">
            {{ $ball->name }} - Tersedia
        </span>
        @endforeach
    </p>
...
<form action="{{route('check-schedule',['field' => $field->id])}}" method="post">
    @csrf
    <div class="w-full" style="color: black; font-weight: 500;">
        <label style="color: black; font-weight: 500;">Hari, Tanggal</label>
        <input type="date" class="bg-white" name="day" placeholder="Hari, Tanggal">
    </div>
    <div class="flex justify-between my-3 space-x-2" style="color: black; font-weight: 500;">
        <div class="w-1/2">
            <label style="color: black; font-weight: 500;">Jam Mulai</label>
            <input type="text" class="bg-white timepicker" name="start_at" placeholder="Jam Mulai" class="timepicker">
        </div>
        <div class="w-1/2">
            <label style="color: black; font-weight: 500;">Jam Selesai</label>
