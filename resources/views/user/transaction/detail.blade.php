...
<div class="my-3 flex justify-between items-center space-x-2 border-b-2 pb-3">
    ...
    <a href="#" onclick="return window.history.go(-1)" class="text-sm text-gray-400 bg-white">
...
<div class="card-transaction">
    <div class="card-header">
        <h2 class="text-dark text-2xl font-medium">
            {{ $transaction->order->bdm_field->name ?? '-' }}
        </h2>
    </div>

    <div>
        <h2 class="text-dark text-lg font-medium">Informasi Jadwal</h2>
        <div class="pl-2">
            <p class="mb-3 text-dark text-sm font-medium">
                <i class="fas fa-xs fa-calendar"></i> {{ $schedule ?? '-' }}
            </p>
            <p class="text-dark text-sm font-medium">
                <i class="fas fa-xs fa-clock"></i>
                {{ $timeStart ?? '-' }} - {{ $timeEnd ?? '-' }} WIB ({{ $transaction->order->hours ?? 0 }} jam)
            </p>
        </div>

        <h2 class="text-dark text-lg font-medium">Informasi Transaksi</h2>
        <div class="pl-2">
            ...
            <div class="flex justify-between items-center">
                ...
                <span>
                    ...
                    <p class="py-1 px-3 text-white text-xs font-medium bg-green-500 rounded-md">Lunas</p>
                    @else
                    <p class="py-1 px-3 text-white text-xs font-medium {{ $transaction->order->status_transaction->color ?? 'bg-gray-400' }} rounded-md">
            ...
            <div class="flex justify-between items-center">
                <p class="my-1 text-gray-600 text-sm font-medium">Subtotal</p>
                <p class="my-1 text-gray-500 text-sm font-medium">
                    {{ $transaction->order->hours ?? 0 }} jam x Rp. {{ number_format($transaction->order->price ?? 0) }}
                </p>
            </div>
            ...
            <div class="flex justify-between items-center">
                ...
                <p class="text-gray-500 text-sm font-medium">
                    Rp. {{ number_format(($transaction->order->hours ?? 0) * ($transaction->order->price ?? 0)) }}
                </p>
            </div>

            <div class="flex justify-between items-center">
                <p class="text-gray-600 text-sm font-medium">Jumlah Tagihan</p>
                <p class="text-green-500 text-xl font-medium">
                    Rp. {{ number_format(($transaction->amount ?? 0) + ($transaction->code ?? 0)) }}
                </p>
            </div>
            ...
            <div class="flex justify-between items-start flex-col space-y-2">
                <p class="text-gray-600 text-sm font-medium">Nomor Rekening</p>
                <p class="text-gray-500 text-sm font-medium">
            ...
            <div class="flex justify-between items-start flex-col space-y-2">
                <p class="text-gray-600 text-sm font-medium">Instruksi Pembayaran</p>
                <div class="text-gray-500 text-sm font-medium leading-8">
                    {!! $transaction->payment_type?->instruction ?? '<em>-</em>' !!}
                </div>
            </div>
            @endif

            @if (($transaction->order->status_transaction_id ?? 0) < 3)
                ...
                <img src="{{ asset("images/logo.png") }}" onerror="previewError()" class="object-cover w-56 border rounded upload cursor-pointer hidden" alt="Foto Bukti Pembayaran" id="preview">
