...
<div class="bg-white shadow-lg px-3 py-2 rounded">
    ...
    <div class="py-2">
        <div class="flex items-start mb-4 flex-col space-y-1">
            <p class="text-sm text-gray-600"><i class="fas fa-th-large"></i> Info Lapangan</p>
            <p class="text-sm text-gray-500">
                {{ optional($order->bdm_field)->name ?? 'Lapangan tidak ditemukan' }}
                ({{ optional(optional($order->bdm_field)->field_type)->name ?? 'Tipe tidak diketahui' }})
            </p>
        </div>
        <div class="flex items-start mb-4 flex-col space-y-1">
    ...
    <div class="card-transaction md:w-1/2 w-full">
        <div class="flex justify-start items-start flex-col">
            ...
            <span>
                ...
                @else
                <p class="py-1 px-3 text-white text-xs font-medium {{ optional($transaction->order->status_transaction)->color ?? 'bg-gray-400' }} rounded-md">
                    {{ optional($transaction->order->status_transaction)->name_user ?? 'Status tidak diketahui' }}
                </p>
                @endif
            </span>
        </div>
        <div class="flex justify-start items-start flex-col">
            ...
            <p class="py-1 px-3 text-white text-xs {{ optional($transaction->transaction_type)->id == 2 ? 'bg-green-500' : 'bg-blue-500' }} rounded-md">
